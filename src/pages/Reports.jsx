...
export default function Reports() {
  ...
  const handleViewMemberDetails = async (id) => {
    try {
      ...
      toast.error('সদস্য তথ্য লোড করা যায়নি');
    }
  };
  ...
  const handleViewProjectDetails = async (id) => {
    try {
      ...
      toast.error('প্রজেক্ট তথ্য লোড করা যায়নি');
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    if (activeTab === 2) {
      ...
      cols = [
        ...
        { header: 'অবশিষ্ট পাওনা', key: 'remainingBalanceFormatted' },
        { header: 'মোট বকেয়া', key: 'totalDueFormatted' },
        { header: 'অবস্থা', key: 'statusFormatted' },
      ];
      dataToExport = reportData.map(row => ({
  ...
  const handleExportPDF = () => {
    if (activeTab === 0) {
      ...
      const dataToExport = [
        ...
        { label: 'আদায়কৃত মোট মুনাফা', value: formatBDT(Math.max(0, (summaryData.totalInstallmentsCollected || 0) - (summaryData.totalInvestments || 0))) },
  ...
  return (
    <div className="app-container">
      ...
      <main className="content-wrapper">
        ...
        {/* ── Driver Ranking (Admin only, tabs 3/4/5) ──────────────── */}
        ...
        {activeTab === 0 && (
          <div>
            {loading ? (
              ...
              const currentBalance = (summaryData.totalDeposits || 0) + (summaryData.totalInstallmentsCollected || 0) - (summaryData.totalInvestments || 0) - (summaryData.totalExpenses || 0);
              const realizedProfit = Math.max(0, (summaryData.totalInstallmentsCollected || 0) - (summaryData.totalInvestments || 0));
              return (
                <div>
                  {/* Hero Balance Card */}
                  <div className="card" style={{ marginBottom: '16px', background: 'linear-gradient(135deg, #1d4ed8 0%, #1e3a8a 100%)', color: 'white', border: 'none', borderRadius: 'var(--radius-lg)', padding: '20px', boxShadow: 'var(--shadow-md)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ flex: 1 }}>
                    ...
                    <div className="table-container" style={{ marginBottom: '0' }}>
                      <table>
                        ...
                        <tbody>
                          ...
                          <tr style={{ backgroundColor: 'rgba(29,78,216,0.08)', borderTop: '2px solid var(--primary)' }}>
                            <td style={{ fontWeight: 800, color: '#1d4ed8' }}>নিট ব্যালেন্স</td>
                            <td style={{ fontWeight: 600 }}>বর্তমান তহবিল ব্যালেন্স</td>
                            <td style={{ textAlign: 'right', fontWeight: 800, fontSize: '1.05rem', color: '#1d4ed8' }}>{formatBDT(currentBalance)}</td>
                          </tr>
        ...
        {/* ═══════════════════════════════════════════════════════════
            TABS 1–6 — Report Tables with tfoot totals
        ═══════════════════════════════════════════════════════════ */}
        {!loading && reportData.length > 0 && activeTab !== 0 && activeTab !== 7 && (
          <div>
            ...
            {memberSummaryData && (
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '10px', marginBottom: '12px' }}>
                  <div className="card" style={{ background: 'linear-gradient(135deg, #1d4ed8, #1e3a8a)', color: 'white', border: 'none', padding: '14px', borderRadius: 'var(--radius-lg)' }}>
